...
export const MapFolderCreate: FC = () => {
  ...
  const schema = yup.object<Partial<FolderDto>>().shape({
    ...
    rank: yup.number().typeError(t('mustBeNumber')).
        required(t('requiredField')) as yup.NumberSchema,
  });
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <CardHeader title={t('maps.addMap')} />
      ...
      <Button type="submit" size="large" sx={{ float: 'right' }} variant="contained"  disabled={createMutation.isPending}
      >{t('create')}</Button>
    </form>
  );
};
