...
type Props = {
  onSubmit: (data: LineFormDataType) => void;
  onClose: () => void;
  editData?: LineFormDataType;
};

const DEFAULT_VALUES: Partial<LineFormDataType> = {
  strokeColorOpacity: 1,
  textSymbolizerFillColorOpacity: 1,
  dashed: false,
};

export const LineForm: FC<Props> = ({ editData, onSubmit, onClose }) => {
  ...
  const schema = object<LineFormDataType>({
  ...
  const methods = useForm<LineFormDataType>({
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {formValues.dashed && <DashedFields methods={methods as UseFormReturn<LineFormDataType>} />}
      </Box>
      <Box
        sx={
